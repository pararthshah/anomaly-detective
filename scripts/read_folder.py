...
class read_folder_lists(read_folder):
    def next(self):
        self.currfile+= 1
        if self.currfile > self.n_files:
            raise StopIteration
        else:
            times, values= read_lists(os.path.join(self.fullpath, self.filelist[self.currfile-1]))
            return (self.filelist[self.currfile-1], times, values)
